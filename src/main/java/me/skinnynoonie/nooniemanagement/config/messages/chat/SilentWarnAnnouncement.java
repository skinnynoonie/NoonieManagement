...
@DefaultMessageConfigValue(defaultValue = "<dark_gray>(Silent) <aqua>{target}<gray> has been <white>warned<gray> by <aqua>{issuer}<gray>.")
public record SilentWarnAnnouncement(NameableUser target, NameableUser issuer) implements ConfigurableMessage {
